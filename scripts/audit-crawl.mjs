...
const DENY = [
  /delete/i, /sign out/i, /log ?out/i, /upgrade/i, /checkout/i, /\bpay\b/i,
  /subscribe/i, /start run/i, /begin run/i, /purchase/i, /restore/i,
  /export/i, /download/i, /remove account/i, /deactivate/i,
];
...
function newRecord(label, path) {
  return { label, path, consoleErrors: [], consoleWarnings: [], pageErrors: [], failedRequests: [], errorBoundary: false, bodyText: '', interactions: [] };
}
...
page.on('pageerror', (e) => { if (cur) cur.pageErrors.push(`${e.message}`.slice(0, 500)); });
page.on('requestfailed', (r) => {
  if (!cur) return;
  const u = r.url();
  // Ignore expected emulator long-poll aborts / analytics noise.
  if (/google-analytics|googletagmanager|firebaseinstallations|fcmregistrations/.test(u)) return;
  cur.failedRequests.push(`${r.failure()?.errorText || 'failed'} ${u}`.slice(0, 300));
});
...
async function checkErrorBoundary() {
  ...
  const eb = /something went wrong|unexpected error|reload the page|this section couldn’t load|this section couldn't load/i.test(txt);
...
async function visit(label, path, { interact = false } = {}) {
  ...
  results.push(cur);
  try {
    await page.goto(BASE + path.replace(/^\//, ''), { waitUntil: 'domcontentloaded', timeout: 20000 });
  } catch (e) {
  ...
  await page.screenshot({ path: `${SHOTS}/${safe}.png`, fullPage: true }).catch(() => {});
  if (interact) await interactButtons(label, path);
}

async function interactButtons(label, path) {
  const handles = await page.locator('button:visible, [role="button"]:visible').elementHandles().catch(() => []);
  const names = [];
  for (const h of handles) {
    const name = ((await h.getAttribute('aria-label')) || (await h.innerText().catch(() => '')) || '').trim().replace(/\s+/g, ' ').slice(0, 40);
    names.push({ h, name });
  }
  const seen = new Set();
  for (const { h, name } of names) {
    ...
    seen.add(name);
    if (DENY.some((re) => re.test(name))) { cur.interactions.push(`SKIP(deny): ${name}`); continue; }
    ...
    try {
      ...
      const dlg = await page.locator('[role="dialog"]:visible, [data-vaul-drawer]:visible, [role="menu"]:visible').count().catch(() => 0);
      if (dlg > 0) {
        const safe = `${label}__${name}`.replace(/[^a-z0-9]+/gi, '_').slice(0, 60);
        await page.screenshot({ path: `${SHOTS}/${safe}.png` }).catch(() => {});
      }
      const after = cur.consoleErrors.length + cur.pageErrors.length;
      cur.interactions.push(`${name}${after > before ? ' [ERROR+]' : ''}${dlg ? ' [opened]' : ''}`);
      ...
      cur.interactions.push(`${name} [click-failed: ${(e.message || '').slice(0, 60)}]`);
    }
  }
}
...
for (const r of results) {
  ...
  console.log(`${flags.length ? '⚠ ' : '  '}${r.label.padEnd(26)} ${flags.join(' ') || 'ok'}`);
}
