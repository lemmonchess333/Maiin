...
function loadAppleRoots() {
  ...
  appleRootsPromise = Promise.all(
    APPLE_ROOT_URLS.map(async (url) => {
      ...
      if (!res.ok) {
        throw new Error(
          `Failed to fetch Apple root cert ${url}: ${res.status}`
        );
      }
...
async function applySubscriptionToUser(uid, signedTransactionInfo) {
  return applePurchase.applySubscriptionToUser({
    ...
    // Sub1 P2.5 — cancel the displaced Stripe sub when an IAP
    // purchase overrides it. Lazy-loaded so a missing STRIPE_SECRET_KEY
    ...
    cancelDisplacedStripeSub: async ({ uid: cancelUid, logger }) => {
      ...
      const stripeAutoCancel = require("./lib/stripeAutoCancel");
      return stripeAutoCancel.cancelDisplacedStripeSub({
        stripe,
        firestore: admin.firestore(),
        uid: cancelUid,
        logger,
...
exports.restoreApplePurchases = functions.https.onCall(
  async (data, context) => {
    ...
    try {
      ...
      console.error(
        `restoreApplePurchases: tombstone lookup failed for uid=${uid}`,
        err
      );
    ...
    try {
      ...
      const latest =
        group && group.lastTransactions && group.lastTransactions[0];
