...
const apply = (state, command) =>
  applyProgramCommand({
    ...
    profile: {},
    now: 123,
    command: { commandId: "queued-before-migration", ...command },
  }).state;
...
describe("commands queued before the Monday migration", () => {
  ...
  it("moves the intended migrated slot to Sunday", () => {
    ...
    expect(moved.runDays[0]).toMatchObject({
      id,
      date: "2026-09-13",
      dayIndex: 0,
      legacyIds: [oldId],
  ...
  it("keeps an exact current ID ahead of another run's alias", () => {
    ...
    expect(after.runDays.map((rd) => rd.status)).toEqual([
      "planned",
      "skipped",
    ]);
  });
  ...
  it("accepts a new ID before the cache-first client's migration write arrives", () => {
    const oldState = {
      ...initial,
    ...
    const after = apply(oldState, {
      kind: "setManualRunCompletion",
      runDayId: "runday_2026-09-07_5_easy_30",
      completed: true,
    });
    expect(after.manualCompletions).toEqual({ [oldId]: { completedAt: 123 } });
    expect(after.runDays[0].id).toBe(oldId);
  });
});
