...
.schedule(“15 23 * * 0”) // Every Sunday at 23:15 UTC
.timeZone(“Europe/London”)
.onRun(async () => {
console.log(“weeklyPerformanceRollup: starting”);

```
// Get active users (lastActiveAt within 30 days)
...
try {
  ...
  // lastActiveAt field may not exist yet — fall back to all users
...
// Backfill guard: skip if workout is outside current week
if (data.date) {
...
// Backfill guard: skip if run is outside current week
if (data.completedAt) {
