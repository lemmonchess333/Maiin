...
**CI runs the same suite four ways, and knowing that changes what you
...
**Test ORDER is swept by hand, not gated** — `npm run test --
--sequence.shuffle` shuffles tests within each file (file-level
shuffling is clean, so nothing leaks between workers). It found five
order-dependent files; three are fixed. The two that remain are
documented where they sit: `WorkoutSessionCompletion.test.tsx` (a
fake-timer group strands jsdom's animation-frame counter AND
motion-dom's batcher — two one-way latches, five fixes measured and
rejected, the fix is its own file) and `useHomeProgram.test.tsx` (its
assertion counts a `vi.mock` factory run, which is once per module
registry). No CI job until those close: a gate that is already red
