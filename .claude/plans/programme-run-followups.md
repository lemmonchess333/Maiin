...
| 22 | Watch onboarding flow on first launch | **Reference apps:** Strava + Apple Workout drop user straight to action (auto-sign-in, JIT permissions); NRC's upfront-permission-wall is the worst-reviewed pattern; Garmin uses hybrid (location early + JIT for the rest); Hevy and Strong both require iPhone counterpart and block standalone Watch use. **Q22a → C with auth sub-states pinned**. Auto-sign-in via shared keychain → Today screen → JIT permissions. Matches Strava + Apple Workout's "get to action fast" model. D's intro screens are friction Strava deliberately avoids; A's "permissions entirely deferred" creates awkward "where do I enable HR?" moments. Launch sequence needs explicit auth sub-states: (1) **keychain has valid Firebase token** → Today screen; (2) **keychain empty / token revoked** → "Open Tropos on iPhone to sign in" screen with `WCSession.isReachable` polling for auto-progress; (3) **iPhone not paired** → Q22d explainer. Critical mechanic: **don't just check `Auth.currentUser != nil`** — Firebase doesn't auto-validate tokens, so a user who signed out on iPhone leaves a non-nil-but-revoked token on Watch. **Call `getIDToken(forcingRefresh: true)` on launch** to catch revoked accounts cleanly. Also **show a 10s spinner during keychain settling** on first install (App Group sync can take seconds) before falling through to "Open on iPhone." **Q22b → C (hybrid: location early, others JIT) with denial fallbacks pinned**. Pure B (all JIT) means tapping Start triggers a modal mid-tap — jarring. Pure A is NRC's upfront-wall. C: location requested ~5s after Today screen stabilises with a one-line in-screen explainer ("Tropos needs location to track your runs · Allow"). **Pin: When-in-use only** — Tropos doesn't need background location; `HKWorkoutSession` grants location-in-background even with WIU for cellular Watch standalone runs. HealthKit requested at **first run save attempt** (not first launch, not HR enable) — rationale dialog "Save this run to Apple Health?" triggers HealthKit prompt if not granted. **Notifications never requested on Watch at all** — Q18a routes all notifications via standard APNs (iPhone-side delivery, Apple auto-routes to Watch), and Watch-local haptics (Q21d interval transitions) are inline not OS-level. Denial fallbacks: **Location denied** → one explainer screen "Tropos needs location to track runs · Open Watch Settings" with `x-apple-watch-settings:` deep link (no re-prompt — violates iOS HIG); **HealthKit denied** → one-time toast "Apple Health save skipped · Enable in Settings later" then graceful degradation (run still saves to Firestore), no re-nagging on subsequent runs. **Q22c → B simplified + accessibility pins**. Single coachmark over whichever primary CTA is visible (Start on run days, "Log a run" on rest days per Q20b). Apple HIG discourages coachmarks generally, but Apple Workout itself pulses workout-type buttons on first launch — exception when affordance isn't obvious. Simplified copy: **"Tap to start a run"** only (drop the long-press hint — overload for first-time users; long-press is discoverable later or never, fine for casual users). Dismiss permanently after first display via shared UserDefaults flag (clears on uninstall, which re-shows on reinstall — fine, reinstall is rare). Hevy briefly tried multi-step coachmarks in 2024 and removed them after user backlash; our C option correctly rejected. Accessibility pins: (1) **respect `UIAccessibility.isReduceMotionEnabled`** — fade-in static arrow instead of pulse for users with vestibular sensitivities; (2) **VoiceOver announcement** via `accessibilityLabel = "Tap to start a run"` + `accessibilityViewIsModal = true` so the coachmark gets focus on screen-load before main content. **Q22d → A with mechanic corrected**. Block-with-explainer when iPhone isn't paired or Tropos isn't installed there. Hevy and Strong both require iPhone counterpart; Tropos needs iPhone for program editing, history, social, nutrition — building a parallel Watch-only sign-in surface is wasted effort for ~99% non-use. **Original draft proposed sending an install-notification to iPhone via WCSession — technically impossible**: WCSession requires *both* sides to have the app installed, so there's no companion extension to receive the message if iPhone Tropos isn't installed. `SKStoreProductViewController` is iOS-only with no Watch equivalent. Final mechanic: **plain text instruction on Watch** ("Install Tropos on your iPhone to continue · Open the App Store and search 'Tropos'") + **`WCSession.isReachable` polling every 30s** for auto-progress when iPhone Tropos launches and is in range (cheap — `isReachable` is a property check, not a network call). State detection: `WCSession.isPaired` distinguishes paired-but-no-app from no-iPhone-at-all; show slightly different copy for each case. **Impact on `tropos-shared` (Q13a):** nothing new — onboarding is platform-specific code, no shared logic. **No PR-L impact.** |
| 23 | Watch settings catalogue & structure | **Reference apps:** Strava + NRC + Hevy + Strong all ship minimal Watch settings (everything else iPhone-only) — the dominant companion-app pattern. Garmin is the outlier (many on-device settings) because Garmin watches are *standalone*, not companions. Apple Workout's split between audio-feedback-volume (per-device) and distance-unit (account-wide) inspires Q23c. **Q23a → A (minimal in-run toggles only) with 4-item catalogue pinned**. Watch settings UX on a tiny screen is slow and error-prone; keep deliberate choices on iPhone. Exceptions are in-run toggles where grabbing iPhone mid-run is hostile. Concrete catalogue: (1) **audio cues on/off**, (2) **voice cue frequency** (every km / every 5min / never), (3) **auto-pause on/off**, (4) **audio cue volume slider** (paired AirPods + Watch speaker). HR display on/off is **deferred** — Q21b's confidence-gating handles it (no user toggle needed; HR shows when reliable, hides when not). Haptic intensity is OS-level (watchOS Settings), not our control. Plus a **"More settings on iPhone →"** row using WCSession message to surface iPhone Settings page (graceful fallback toast when iPhone unreachable, same pattern as Q20d "View all on iPhone"). 4 items fits one screen, no scroll. **Q23b → A (24pt gear icon, top-right of Today)**. Force Touch (B) was deprecated in watchOS 7 and the hardware is gone Series 7+; Apple HIG says don't use anywhere now. C is mixed convention. A is the modern pattern. Pins: **24pt gear icon top-right** (inline with time indicator, doesn't shrink hero); **VoiceOver tab order explicit** — gear announces *after* main content (hero, Start, water, week) since settings is secondary to today's run. **Q23c → C (split by category) with explicit rule and category table**. Pure A (iPhone canonical everywhere) forces Watch to write through to Firestore on every toggle — slow on cell, broken when offline. Pure B is confusing (user expects "km" to mean km everywhere). C splits by user expectation: **"consistency expected across devices = data preference (Firestore-stored); per-device personalisation expected = display preference (local UserDefaults)."** Concrete category table pinned to `tropos-shared`: **data** = distance unit, weight unit, voice cue language, voice cue voice (Sam/Alex), race goal, program preferences — *voice cue language/voice goes to data despite being technically local-file selection* because users expect consistency. **display** = audio cues on/off, audio cue volume, auto-pause on/off, voice cue frequency. Apple Workout makes this exact split (audio feedback volume per-device, distance unit account-wide). No migration needed — Watch is new, reads existing iPhone settings from Firestore on launch (per Q17a read-through). **Q23d → A (sign-out available on Watch) with three pins**. B is hostile (user wants to sign out on the Watch they're holding *now*); C is destructive (could disconnect iPhone unexpectedly, breaking flow). A: explicit confirm sheet → routes to Q22a's "Open Tropos on iPhone to sign in" screen. Pins: (1) **confirm sheet copy** — "Sign out of Tropos on this Watch? · You'll need to sign in again on iPhone to use Tropos here." (makes Watch-scope + iPhone re-sign-in requirement explicit); (2) **pre-flight check for pending writes** — Watch may have a queued run that hasn't synced; if non-empty, warn "X items not yet synced — sign out anyway? Data will be lost." User chooses (Z option, not discard-silently nor block-until-drained); (3) **keychain cleared on Watch only** — App Group keychain entry for Firebase token wiped on Watch; iPhone keychain unaffected per Apple's per-device-keychain design. Multi-account support automatic via Q22a's `getIDToken(forcingRefresh: true)` on next launch. **Impact on `tropos-shared` (Q13a):** beyond the `SettingsCategory` enum, pin the **concrete category table** (which keys are `.data` vs `.display`) as a shared constant so Watch and iPhone route storage identically without drift. **No PR-L impact** — data settings live in profile doc, already covered by PR-L's profile LWW rule from Q15c. |
| 24 | Watch app testing strategy | **Reference apps:** Strava runs a physical Watch lab (gold standard but multi-month investment); NRC uses paid contractor dogfood + TestFlight (mid-tier); Hevy is TestFlight-only with founder publicly discussing the "small team, ship fast, accept some field bugs" trade-off; Strong similar to Hevy. Tropos's team scale puts us in the NRC / Hevy hybrid camp; Strava's lab approach explicitly rejected as over-investment for our scale. **Q24a → D (unit + iOS simulator + manual device-test checklist) with three pins**. C (full automated device CI) is what Strava can afford but multi-month investment we can't justify pre-launch. A (unit only) is too thin — iOS simulator tests catch a lot of iPhone-side regressions cheaply. (1) **Concrete 10-item checklist** lives at `docs/testing/watch-device-checklist.md` as source of truth (PR template links to it), split into **8 per-PR items (fast, ~1hr)** — GPS accuracy on one outdoor run, Apple Health workout write, connectivity-drop survives mid-run, cross-device clientRunId tiebreaker, haptic patterns distinguishable, cellular phone-free run, disk checkpoint after force-quit mid-run, keychain sync — and **2 weekly items (slow, multi-hour)** — 24h background-refresh budget burn, full WatchConnectivity state matrix; (2) **Screen recording (any tool — Loom, QuickTime, etc.)** required for run-related items in PR description; desk-test items get text confirmation; (3) **Simulator UI coverage targets**: 60% global, 80% for critical paths (auth flow, run logging, food parsing, run save). **Q24b → D (hybrid: staff dogfood + TestFlight cohort) with four pins**. A alone is slow; B (physical lab) is overkill; C alone misses fast-iteration day-to-day issues. (1) **Mandatory weekly dogfood on the week a Watch-touching PR merges** (optional otherwise); (2) **8-scenario structured dogfood**: scheduled-run start, ad-hoc-run start (long-press Start), pause/resume mid-run, long-press end, splits screen via crown, phone-free run, Today widget updates post-run, sign out + sign back in; **PR author appends 1-2 scenario-specific checks** tailored to the PR's change; (3) **Watch in-app "Send feedback" button writes to Firestore `feedback/{uid}/watch/{auto-id}` collection** (works offline via Q15a queue, no iPhone dependency unlike WCSession approach — team reviews via internal admin tool); (4) **TestFlight cohort target 50 recruited via in-app banner shown to users with ≥5 completed runs** (signals engagement, filters tyre-kickers); **promotion gate at ≥25 completed a full run cycle on the build**; cohort communicated upfront to "expect 1-2 builds per week." **Q24c → C (tagged tests) with three pins**. A is naive — HealthKit fails silently on simulator; B is too rigid. C uses Swift Testing's tag traits — `@Test(.tags(.simulatorOnly))` / `.deviceOnly` / `.either`. (1) **Xcode 16+ minimum** dev environment (tag traits unsupported below); (2) **macOS GitHub Actions runner for simulator CI** (~$12/month at Tropos PR volume — trivial; watch concurrency on free tier); **device-only tests are manual** on developer machines, no nightly device CI for v1; (3) **Monthly device-test sweep on Series 4 + Series 9** (low-end + current); **minimum supported = Series 4 / watchOS 10**, aligning with Apple's official baseline. Default to `@deviceOnly` when uncertain (avoids CI noise from flaky simulator-incompatible tests). **Q24d → A+B narrowed (staff smoke gates TestFlight, TestFlight gates production) with three pins**. Pure A misses field issues; pure B is slow. Sequence: ship to TestFlight only after **staff smoke passes**; promote TestFlight to App Store only when **(i) 14-day TestFlight window elapsed AND (ii) zero `severity-critical` bugs AND (iii) ≥10 testers completed a full run cycle on the build**. Cosmetic bugs don't block — only criticals do. Pins: (1) **`watch_run_completed` telemetry event (aggregate-only, counts not per-user identifiers)** for the ≥10 metric; **user-identifiable analytics opt-in lives on iPhone under privacy settings** (per Q23c — analytics is a `.data` category preference, iPhone-canonical, Watch respects iPhone's setting); (2) **`severity-critical` GitHub label**: any team member can apply, release manager (PR author by default) confirms; **critical-fix triggers a fresh 7-day TestFlight window** (not full 14), **release manager can extend** for soak-time-dependent fixes (e.g. battery-drain regressions need 5+ days); (3) **Two-tier hotfix gate** when production is critically broken: **standard hotfix = 48h TestFlight + ≥3 testers complete a run on the build**; **expedited hotfix = 12h TestFlight + staff member completes a run on the build + screen-recording proof of fix** (use only when production is critically broken, e.g. app crashes on launch). **Impact on `tropos-shared` (Q13a):** the package's `Tests` target is **the first build target** — gates all subsequent Watch work. **≥80% line coverage** required on the locked-decision modules: pace smoothing + 30s rolling average (Q14b), `recoveryWeeksForDistance` (Q10a), `IntervalState` transitions (Q21d), haptic phase constants (Q21d), settings category table (Q23c), state enums (Q19), `TodayRunSnapshot` (Q18), PR classification (Q18d), Watch-wins tiebreaker constants (Q16d). No Watch UI work begins until the shared package hits the bar. **PR-L impact:** Cloud Function reconciliation tested independently via Firebase Functions emulator + test framework — separate concern from Watch testing but worth flagging as a parallel testable surface. |
