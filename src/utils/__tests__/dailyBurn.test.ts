...
describe("calcDailyBurn", () => {
  ...
  // profile.targetCalories as the target. That value already includes the
  // user's activity-level-aware TDEE and the phase deficit (from
  // calculateTDEE). Completed activity (workouts, runs, steps) is carried
  // through for INFORMATIONAL display only — it is NEVER added to the target
  // (no eat-back budget).

  it("reports the target unchanged when no activity is supplied", () => {
    const result = calcDailyBurn(1887, "cut", 0, 0, 0);
    expect(result.phaseAdjustedTdee).toBe(1887);
    expect(result.phaseLabel).toBe("cut");
  });

  it("does NOT add workout calories to the target (no eat-back)", () => {
    const result = calcDailyBurn(2400, "lean bulk", 400, 0, 0);
    expect(result.phaseAdjustedTdee).toBe(2400);
    expect(result.workoutCalories).toBe(400);
    expect(result.phaseLabel).toBe("bulk");
  });

  it("carries all activity sources for display without touching the target", () => {
  ...
  it("exposes no eat-back budget field", () => {
    const result = calcDailyBurn(2200, "recomp", 300, 200, 100);
    expect("dailyBudget" in result).toBe(false);
  });
