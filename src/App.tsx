...
function AppRoutes() {
  ...
  return (
    /* HOME-ACCOUNT-01: remount the entire authenticated subtree when the
       account changes, so no provider's cached Firestore state or page-local
       React state can cross from account A to account B on a shared browser
       (an account switch doesn't reload the WebView on native). */
    <AuthSessionBoundary uid={user.uid}>
      <Suspense fallback={<PageLoader />}>
