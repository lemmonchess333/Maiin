...
export default function WeightStepsTiles({ lastWeight, weightUnit, onLogWeight, lastWeightDate }: {
  ...
  return (
    <div className="grid grid-cols-2 gap-2">
      <button onClick={function() { haptic(); trackHomeEvent("home_card_tapped", { card: "weight" }); onLogWeight(); }} className="p-3 rounded-xl text-left active:scale-[0.97] bg-muted relative">
      ...
      <button onClick={function() { haptic(); trackHomeEvent("home_card_tapped", { card: "steps" }); }} className="p-3 rounded-xl text-left active:scale-[0.97] bg-muted group">
        <div className="flex items-center gap-2 mb-1.5">
          <div className="w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0" style={{ backgroundColor: THEME.iconBg }}>
