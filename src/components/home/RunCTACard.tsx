...
export default function RunCTACard({ todayRun, navigate, isPrimary = true }: {
  ...
  return (
    <motion.button whileTap={{ scale: 0.97 }}
      ...
      <div className="flex items-center gap-3">
        <div className={`${isPrimary ? 'w-10 h-10' : 'w-8 h-8'} rounded-lg flex items-center justify-center`} style={{ backgroundColor: THEME.running + "18" }}>
          <RunIconComp className={isPrimary ? "w-5 h-5" : "w-4 h-4"} style={{ color: THEME.running }} />
        </div>
        <div className="flex-1 min-w-0">
          <p className="text-micro uppercase tracking-wider mb-0.5" style={{ color: THEME.running }}>Today · Run day</p>
          <div className="flex items-baseline gap-2">
            <p className={`${isPrimary ? 'text-sm' : 'text-xs'} font-bold text-foreground truncate`}>{runLabel}</p>
            {isPrimary && runKeyMetric && (
              <span className="text-sm font-bold font-mono tabular-nums" style={{ color: THEME.running }}>{runKeyMetric}</span>
            )}
          </div>
          {isPrimary && <p className="text-micro text-muted-foreground truncate">{runDesc}</p>}
        </div>
        <div className={`flex items-center gap-1.5 ${isPrimary ? 'px-3.5 py-2' : 'px-2.5 py-1.5'} rounded-lg text-xs font-bold shadow-sm`} style={{ background: `linear-gradient(135deg, ${THEME.running}, ${THEME.runningLight})`, color: "white" }}>
