...
export default function ShoesManager() {
  ...
  return (
    <div className="space-y-3">
        ...
        <div key={shoe.id} className="p-3 rounded-xl bg-card border border-border space-y-2">
          <div className="flex items-center justify-between">
            <div>
              ...
              {shoe.brand && <p className="text-[10px] text-muted-foreground">{shoe.brand}</p>}
            </div>
      ...
      {retired.length > 0 && (
        <div className="pt-2">
          <p className="text-[10px] text-muted-foreground font-medium uppercase tracking-widest mb-2">Retired</p>
          {retired.map((shoe) => (
            <div key={shoe.id} className="flex items-center justify-between py-2 px-3 rounded-lg bg-muted/30 mb-1 opacity-60">
              <div>
                ...
                <p className="text-[10px] text-muted-foreground">{Math.round(shoe.totalKm)} km logged</p>
              </div>
            </div>
          ))}
        </div>
