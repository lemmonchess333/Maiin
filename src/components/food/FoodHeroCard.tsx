    ...
    <>
      {compact ? (
        <section
          ...
          aria-label="Daily energy summary"
        >
          <div className="flex items-center justify-between gap-2">
            <p className="text-sm text-muted-foreground">
              {caption?.trainingType ?? "Daily energy"}
            </p>
            <div className="flex items-center gap-1">
              ...
              <Link
                ...
                className="size-11 flex items-center justify-center rounded-lg text-muted-foreground hover:bg-muted focus-visible:ring-2 focus-visible:ring-primary"
              >
          ...
          <div className="flex items-center justify-between gap-3">
            <Button
              variant="ghost"
              className="h-auto min-h-11 px-0 justify-start text-left"
              onClick={toggleMode}
              aria-label={`Show calories ${mode === "left" ? "eaten" : "left"}`}
            >
              <span>
                <span className="font-mono tabular-nums text-2xl font-bold">
                  {Math.round(
                    mode === "eaten"
                      ...
                      : Math.abs(
                          dailyTargets.finalTarget - dailyTotals.calories
                        )
                  ).toLocaleString()}
                </span>
                <span className="ml-2 text-sm font-normal text-muted-foreground">
                  ...
                  {mode === "eaten"
                    ? "eaten"
                    : dailyTotals.calories > dailyTargets.finalTarget
                      ? "over target"
                      : "left"}
                </span>
                <span className="block text-caption font-normal text-muted-foreground">
                  ...
                  <span className="font-mono tabular-nums">
                    {Math.round(dailyTargets.finalTarget).toLocaleString()}
                  </span>{" "}
                  kcal
                </span>
              </span>
          ...
          <div className="grid grid-cols-3 gap-2 mt-4">
            <div className="min-w-0 flex p-3 rounded-2xl bg-card card-shadow">
              <MacroColumn
                ...
                target={
                  dailyTargets.targetInfeasible ? 0 : dailyTargets.protein
                }
                label="PROTEIN"
