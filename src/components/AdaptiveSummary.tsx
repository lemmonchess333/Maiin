...
const phaseConfig = {
  "lean bulk": { calorieMultiplier: 1.1, proteinRatio: 2.2, fatRatio: 0.25, plateauSensitivity: 1 },
  cut: { calorieMultiplier: 0.85, proteinRatio: 2.4, fatRatio: 0.3, plateauSensitivity: 0.8 },
  recomp: { calorieMultiplier: 1, proteinRatio: 2.3, fatRatio: 0.25, plateauSensitivity: 1.2 },
  "strength peak": { calorieMultiplier: 1.15, proteinRatio: 2.2, fatRatio: 0.25, plateauSensitivity: 1.5 },
};
...
function detectPlateau(avgLiftChange: number, avgWeightChange: number) {
  if (avgLiftChange === 0 && avgWeightChange === 0) return { message: "Performance stagnant. Increase calories by ~150.", volumeAdjustment: 0 };
  if (avgWeightChange > 0.4 && avgLiftChange === 0) return { message: "Weight rising without strength gains. Shift macros toward carbs.", volumeAdjustment: 0 };
  if (avgLiftChange < 0) return { message: "Strength declining. Reduce volume by 10% this week.", volumeAdjustment: -0.1 };
  return { message: "Progress trending well. Stay consistent.", volumeAdjustment: 0 };
}

/* ================================
   INTELLIGENT MACRO ENGINE
================================ */
function calculateAdaptiveMacros(bodyweight: number, avgLiftChange: number, avgWeightChange: number, phase: PhaseMode) {
  ...
  return { calories: Math.round(adjustedCalories), protein: Math.round(protein), carbs: Math.round(carbs), fat: Math.round(fats) };
}
...
export function AdaptiveSummary({
  ...
  monthlyWorkoutsTarget = 0,
  monthlyMealsDone = 0,
  monthlyMealsTarget = 0,
  monthlyPR = false,
  ...
  const avgWeightChange = bodyweightTrend.slice(-3).reduce((a, b) => a + b, 0) / Math.max(bodyweightTrend.slice(-3).length, 1) || 0;
    ...
    <div className="bg-card rounded-2xl border border-border/50 overflow-hidden">
      <div className="p-5 space-y-5">
        ...
        <div className="flex justify-between items-center">
          <h3 className="font-semibold text-foreground">Performance Engine</h3>
          {isPro && (
            <select
              value={phase}
        ...
        {!isPro && (
          <div className="p-4 rounded-xl bg-muted/30 border border-border text-center space-y-3">
            ...
            <p className="text-xs font-semibold">£{pricing.monthly}/month or £{pricing.yearly}/year</p>
          </div>
        )}
