  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(
      ...
      (snapshot) => {
        ...
        setLoading(false);
        // Activation funnel — first food logged (once/user). Fires when the
        // user first has any meal; covers every Food.tsx logging path without
        // instrumenting each. trackFirst dedups via a uid-scoped guard.
        if (data.length > 0) trackFirst("first_food_logged", user.uid);
      },
