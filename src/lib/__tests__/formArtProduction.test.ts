...
// Synthetic reviewed v2 scene: legacy production scene files remain blocked.
const scene: FormArtScene = {
  ...curl as FormArtScene,
  athleteVersion: "anatomy-shorts-v2",
...
describe("six-frame production brief", () => {
  it("uses the exact six authored cues and canonical reference", () => {
    ...
    expect(prompt).toContain("docs/exercise-art/identity/athlete-shorts-v2.png");
  ...
  it("rejects the old athlete and another exercise's scene master", () => {
    expect(() => buildFormArtPrompt("db-curl", curl as FormArtScene)).toThrow(/canonical athlete in shorts/);
  ...
  it("includes a physical selected-stack ladder and rejects reversed movement", () => {
    ...
    plan.athleteVersion = "anatomy-shorts-v2";
