...
describe("inventory shape — Chunk 1.1 schema v2", () => {
  ...
  it("designConstants document the spec items 15-19", () => {
    ...
    expect(inventory.designConstants.verificationRule).toContain(
      "strongly consistent"
    );
    expect(inventory.designConstants.systemWriterCheckTiming).toContain(
      "before each write commit"
    );
    expect(inventory.designConstants.counterIdempotence).toContain(
      "transaction"
    );
    expect(inventory.designConstants.lockFirstEnumerateSecond).toContain(
      "OUTSIDE"
    );
  });
});
...
describe("included entries — required fields and allowed values", () => {
  it("every included entry has the full Chunk 1.1 field set", () => {
    for (const entry of inventory.included as IncludedEntry[]) {
      ...
      expect(ALLOWED_TEST_COVERAGE_STATUSES).toContain(
        entry.testCoverageStatus
      );
    }
  });
  ...
  it("naming-drift entries carry the expected legacy aliases", () => {
    ...
    expect(foodFav!.aliases!.map((a) => a.path)).toContain(
      "users/{uid}/favorites"
    );

    const settings = findEntry("userSettings");
    expect(settings!.aliases!.map((a) => a.path)).toContain(
      "users/{uid}/preferences"
    );
  });

  it("userPublicSubcollection (not userPublicProfile) is the public-data entry", () => {
    const pub = (inventory.included as IncludedEntry[]).find(
      (e) => e.key === "userPublicSubcollection"
    );
    expect(
      pub,
      "userPublicSubcollection must exist (replaces userPublicProfile)"
    ).toBeTruthy();
    expect(pub!.path).toBe("users/{uid}/public");
    expect(pub!.strategy).toBe("deleteSubcollection");
    const oldKey = (inventory.included as IncludedEntry[]).find(
      (e) => e.key === "userPublicProfile"
    );
  ...
  it("userRoot is documented as having nested subcollections handled separately", () => {
    const root = (inventory.included as IncludedEntry[]).find(
      (e) => e.key === "userRoot"
    );
    expect(root!.hasNestedSubcollections).toBe(true);
    expect(
      (root as unknown as { nestedSubcollectionsHandledBy: string })
        .nestedSubcollectionsHandledBy
    ).toBeTruthy();
  });

  it("activitiesOwn audit note confirms no nested subcollections under activities/{id}", () => {
    const act = (inventory.included as IncludedEntry[]).find(
      (e) => e.key === "activitiesOwn"
    );
    expect(act!.hasNestedSubcollections).toBe(false);
    expect(
      (act as unknown as { nestedSubcollectionsAuditNote: string })
        .nestedSubcollectionsAuditNote
  ...
  it("activitiesOwnKudos and activitiesOwnComments exist for content-ID reference cleanup", () => {
    const kudosRef = (inventory.included as IncludedEntry[]).find(
      (e) => e.key === "activitiesOwnKudos"
    );
    const commentsRef = (inventory.included as IncludedEntry[]).find(
      (e) => e.key === "activitiesOwnComments"
    );
    expect(kudosRef).toBeTruthy();
    expect(commentsRef).toBeTruthy();
    expect(
      (kudosRef as unknown as { preflightSource: string }).preflightSource
    ).toContain("activities where authorId");
    expect(
      (commentsRef as unknown as { preflightSource: string }).preflightSource
    ).toContain("activities where authorId");
  });
});
...
describe("excluded entries — required fields and allowed values", () => {
  it("every excluded entry has the Chunk 1.1 schema", () => {
    for (const entry of inventory.excluded as ExcludedEntry[]) {
      ...
      if (entry.containsPersonalData) {
        ...
        const oneWordRejects =
          /^(operational|system|legal|business|required)\.?$/i;
  ...
  it("reportsAboutMe uses anonymiseMinimisedShape (not retain unchanged)", () => {
    const reports = (inventory.excluded as ExcludedEntry[]).find(
      (e) => e.key === "reportsAboutMe"
    );
    ...
    expect(
      (
        reports as unknown as {
          minimisedShape: { abuseIdentityHashSpec: string };
        }
      ).minimisedShape.abuseIdentityHashSpec
    ).toContain("HMAC");
  });
  ...
  it("commentsAuthoredByMe defines Option C interim: visible 'Comment deleted' + server-only originalText for moderation reversibility", () => {
    ...
    const anonFields = (
      comments as unknown as { anonymiseFields: Record<string, string | null> }
    ).anonymiseFields;
    ...
    const renderAudit = (
      comments as unknown as {
        renderSafetyAudit: {
          verdict: string;
          renderers: unknown[];
          originalTextLeakageAudit: string;
        };
      }
    ).renderSafetyAudit;
  ...
  it("paymentEventsPostDeletion is NOT marked as indefinite default retention", () => {
    const pmt = (inventory.excluded as ExcludedEntry[]).find(
      (e) => e.key === "paymentEventsPostDeletion"
    );
    expect(pmt!.retentionWindow.toLowerCase()).not.toBe("indefinite");
    expect(pmt!.retentionWindow).toContain("90 days");
    expect(
      (pmt as unknown as { reviewClosureProcess: string }).reviewClosureProcess
    ).toBeTruthy();
  });

  it("deletedBillingIdentities is a separate billing tombstone", () => {
    const billing = (inventory.excluded as ExcludedEntry[]).find(
      (e) => e.key === "deletedBillingIdentities"
    );
    expect(
      billing,
      "deletedBillingIdentities must exist as a separate billing tombstone"
    ).toBeTruthy();
  ...
  it("operational records (Z_operational) all carry minimisation rules", () => {
    ...
    for (const entry of operational) {
      expect(
        entry.minimisationRule,
        `${entry.key} missing minimisationRule`
      ).toBeTruthy();
    }
  });
  ...
  it("PII classification corrections: ledger / tombstone / paymentEvents are containsPersonalData:true even though they were 'no' in Chunk 1", () => {
    const keys = [
      ...
      "deletedAccountsTombstone",
      "paymentEventsPostDeletion",
    ];
    for (const key of keys) {
      const entry = (inventory.excluded as ExcludedEntry[]).find(
        (e) => e.key === key
      );
      expect(
        entry!.containsPersonalData,
        `${key} should have containsPersonalData: true (uid or paymentId is PII)`
      ).toBe(true);
    }
  });
  ...
  it("scanUsage and userEngineLock are also containsPersonalData:true (uid is PII per piiCategory:uid)", () => {
    ...
    for (const key of keys) {
      const entry = (inventory.included as IncludedEntry[]).find(
        (e) => e.key === key
      );
      expect(
        entry!.containsPersonalData,
        `${key} should have containsPersonalData: true`
      ).toBe(true);
