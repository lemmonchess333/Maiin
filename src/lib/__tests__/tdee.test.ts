...
describe("calculateTDEE", () => {
  ...
  it("distributes macros correctly", () => {
    ...
    const expectedFat = Math.round(Math.round(result.targetCalories * 0.25) / 9);
    expect(result.fat).toBe(expectedFat);
    // Carbs: remainder / 4
    const proteinCals = 160 * 4;
    const fatCals = Math.round(result.targetCalories * 0.25);
    const expectedCarbs = Math.round(Math.max(0, result.targetCalories - proteinCals - fatCals) / 4);
