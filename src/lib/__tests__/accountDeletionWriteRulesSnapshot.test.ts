...
const EXPLICITLY_EXEMPT = [
  {
    ...
    reason:
      "Cross-user write — kudos giver is request.auth.uid. Freeze would require !isDeleting(userId), but kudos creation rate is high. NOTE (2026-08-12): this exemption used to be justified by 'the Chunk 3 executor sweeps kudosByMe via collectionGroup'. That sweep does not exist and CANNOT be written as specified: collectionGroup(...).where(documentId(), '==', uid) is rejected by Firestore, which requires a full document path for a collection-group documentId comparison (see firestore.collectionGroup.test.ts, which pins the rejection against the emulator). kudosByMe is additionally unreachable by field query because socialCounters.toggleKudos writes the doc as { createdAt } only — the uid lives solely in the path — and its parent, kudos/{activityId}, is unbounded. Closing it needs a uid field at the write site plus a backfill. The exemption still stands on the rate argument alone; the correction is recorded here rather than swapped out because a rules decision resting on a cleanup path that cannot exist is the thing worth seeing.",
  },
  {
...
const INFRASTRUCTURE_AND_READ_ONLY = [
  ...
  "match /accountDeletionWork/{uid}",
  "match /accountDeletionBilling/{uid}",
  "match /deletedCommentEvidence/{id}",
