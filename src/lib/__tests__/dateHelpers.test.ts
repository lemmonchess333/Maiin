...
describe("localWeekKey", () => {
  it("returns the Monday on or before the given date", () => {
    ...
    expect(localWeekKey(wed)).toBe("2026-05-11"); // Monday May 11
  });
  ...
  it("crosses month boundaries correctly", () => {
    // Fri May 1 belongs to the week beginning in April.
    const fri = new Date(2026, 4, 1);
    expect(localWeekKey(fri)).toBe("2026-04-27");
    // Mon Jun 1 begins a new week and month
...
describe("localWeekKey (Monday anchor, literal dates)", () => {
  ...
  it("returns the same date for a Monday", () => {
    // 2025-01-06 is a Monday → Monday is 2025-01-06
