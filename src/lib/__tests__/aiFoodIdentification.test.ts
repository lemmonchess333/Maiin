...
describe("isEmptyAiFoodResult", () => {
  ...
  it('returns false for "Black coffee" with zero macros (real zero-cal food)', () => {
    expect(
      isEmptyAiFoodResult([
        { name: "Black coffee", calories: 0, protein: 0, carbs: 0, fat: 0 },
      ])
    ).toBe(false);
  });
...
/* ── Prompt contract: the server's no-food sentence and this filter
   must agree ───────────────────────────────────────────────────────
   analyzeFood's prompt (functions/index.js) instructs the model:
   when the image has no food, return foodName "No food detected"
   with items []. That exact name is what routes a non-food photo
   into the modal's "No food detected" failure beat — via THIS
   module's name filter. The two literals live a repo apart; this is
   the pin that makes them one decision (the mirror-parity rule:
   reachability over prose — same shape as engineMirror's literal
   pins). Reword the prompt and this fails until the filter (or the
   pin) moves with it. */
describe("promptContract — analyzeFood no-food sentence", () => {
  it("the prompt instructs a no-food fallback and the filter catches its exact name", async () => {
    const { readFileSync } = await import("node:fs");
    const { fileURLToPath } = await import("node:url");
    const { dirname, resolve } = await import("node:path");
    const here = dirname(fileURLToPath(import.meta.url));
    const source = readFileSync(
      resolve(here, "../../../functions/index.js"),
      "utf8"
    );
    ...
    /* …and the exact foodName it promises must be one the client
       filter treats as generic — otherwise a non-food photo renders
       as a real result with that name. */
    expect(isGenericAiFoodName(match![1])).toBe(true);
    /* Belt AND braces: items:[] alone must also read as empty. */
    expect(isEmptyAiFoodResult([])).toBe(true);
  });
});
