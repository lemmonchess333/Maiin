...
describe('generateInsight', () => {
  it('includes strength trending up for up momentum', () => {
    const insight = generateInsight(makeInsightData({ momentumDir: 'up', fourWeekPct: 5 }));
    expect(insight).toMatch(/trending up/i);
  });

  it('includes strength declining for down momentum', () => {
    const insight = generateInsight(makeInsightData({ momentumDir: 'down', fourWeekPct: -5 }));
    expect(insight).toMatch(/declining/i);
  });

  it('includes stable message for flat momentum', () => {
    const insight = generateInsight(makeInsightData({ momentumDir: 'flat', fourWeekPct: 0 }));
    expect(insight).toMatch(/stable/i);
  });

  it('advises increasing intake for low surplus in lean bulk', () => {
    const insight = generateInsight(makeInsightData({ phase: 'lean bulk', avgCalorieDiff: 50 }));
    expect(insight).toMatch(/surplus is low/i);
  });

  it('advises about weight drop during bulk', () => {
    const insight = generateInsight(makeInsightData({
      phase: 'lean bulk', avgCalorieDiff: 250, bodyweightTrend: -0.5, momentumDir: 'flat',
    }));
    expect(insight).toMatch(/dropping/i);
  });

  it('warns about strength loss during cut', () => {
    const insight = generateInsight(makeInsightData({ phase: 'cut', momentumDir: 'down', fourWeekPct: -3 }));
    expect(insight).toMatch(/strength loss/i);
  });

  it('praises solid execution during cut with maintained strength', () => {
    const insight = generateInsight(makeInsightData({ phase: 'cut', momentumDir: 'up', fourWeekPct: 2 }));
    expect(insight).toMatch(/solid/i);
  });

  it('warns about weight trending up during cut', () => {
    const insight = generateInsight(makeInsightData({ phase: 'cut', momentumDir: 'up', bodyweightTrend: 0.5 }));
    expect(insight).toMatch(/weight trending up/i);
  });

  it('notes strong adherence for recomp', () => {
    const insight = generateInsight(makeInsightData({ phase: 'recomp', weeklyAdherence: 90 }));
    expect(insight).toMatch(/favorable/i);
  });

  it('notes consistency needed for recomp with low adherence', () => {
    const insight = generateInsight(makeInsightData({ phase: 'recomp', weeklyAdherence: 60 }));
    expect(insight).toMatch(/consistency/i);
  });

  it('notes peak phase progress', () => {
    const insight = generateInsight(makeInsightData({ phase: 'strength peak', momentumDir: 'up' }));
    expect(insight).toMatch(/peak phase/i);
  });

  it('suggests deload for stalling peak', () => {
    const insight = generateInsight(makeInsightData({ phase: 'strength peak', momentumDir: 'flat', fourWeekPct: 0 }));
