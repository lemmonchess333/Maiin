...
import { parseDailyLog, stripUndefined } from "../firestoreGuards";
...
describe("parseDailyLog", () => {
  ...
  it("handles non-numeric workouts/meals gracefully", () => {
    const result = parseDailyLog("doc3", {
      workouts: "two",
      meals: null,
      weightKg: "heavy",
    });
...
describe("stripUndefined", () => {
  ...
  it("preserves null values (Firestore accepts them as a meaningful signal)", () => {
    expect(stripUndefined({ shoeId: null, distance: 1000 })).toEqual({
      shoeId: null,
      distance: 1000,
    });
  });
  ...
  it("walks arrays and replaces undefined elements with null to preserve length", () => {
    expect(stripUndefined({ splits: [1, undefined, 3] })).toEqual({
      splits: [1, null, 3],
    });
  });
