...
import { getInvalidRunReason, MAX_PLAUSIBLE_SPEED_MS } from "../runGuards";
...
describe("validity agrees across the pace/duration/distance floors", () => {
  /* This block used to pin `isInvalidRun` against `getInvalidRunReason`,
   * proving the boolean wrapper was a thin convenience over the typed
   * reason. The wrapper was deleted 2026-07-27 as unreachable, which
   * removes the subject but NOT these four cases — each is a real
   * threshold combination (fat-fingered 20km/0:08 treadmill, a valid
   * long treadmill run, sub-30s outdoor, valid outdoor). They are kept
   * on the surviving predicate rather than deleted with the wrapper. */
  it.each([
    {
    ...
    "agrees on $activityType $distanceKm/$elapsedSeconds → $expected",
    ({ activityType, distanceKm, elapsedSeconds, expected }) => {
      expect(
