 ...
 * say so with `SPLITS_ARE_PER_KM` / `GENERATION_TIME_UNIT` at the point of
 * use, which reads as a decision rather than an omission.
 */
  ...
  it("no display surface formats a distance by hand", () => {
    ...
    expect(
      ...
      `Format run distances with distanceLabel / distanceLabel2 / ` +
        ...
        `point of use with SPLITS_ARE_PER_KM or GENERATION_TIME_UNIT:\n  ` +
        hits.join("\n  ")
    ).toEqual([]);
  });
