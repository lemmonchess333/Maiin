...
/** Tuesday 14:00 LOCAL. Week starts Sunday, so: this week from Sun 26th;
...
function run(localTime: string, km: number) {
  return {
    completedAt: Timestamp.fromDate(new Date(localTime)),
    distance: km * 1000,
    duration: 60,
  };
}
...
describe("getPersonalTrajectory", () => {
  ...
  it("includes the local week start but excludes the exact to-date endpoint", async () => {
    seedFirestore({
      "users/user1/runs/start": run("2026-04-19T00:00:00", 1),
      "users/user1/runs/before_slice_end": run("2026-04-21T13:59:59", 2),
      "users/user1/runs/at_slice_end": run("2026-04-21T14:00:00", 4),
      "users/user1/runs/next_week": run("2026-04-26T00:00:00", 8),
    });

    const result = await getPersonalTrajectory("user1");
    expect(result.lastWeekToDate.km).toBe(3);
    expect(result.lastWeek.km).toBe(7);
    expect(result.thisWeek.km).toBe(8);
  });
