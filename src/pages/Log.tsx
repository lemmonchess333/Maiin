...
```
  <div className="flex items-center justify-between bg-card rounded-xl border border-border/50 p-3">
    <button onClick={() => changeDate(-1)} className="p-2 rounded-lg hover:bg-muted transition-colors">
    ...
    <button
      ...
      disabled={isToday}
      className={cn("p-2 rounded-lg transition-colors", isToday ? "opacity-30 cursor-not-allowed" : "hover:bg-muted")}
    >
  ...
  <div className="flex gap-2 bg-muted rounded-xl p-1">
    <button
      ...
      className={cn(
        ...
        activeTab === "workout" ? "bg-card text-foreground shadow-sm" : "text-muted-foreground"
      )}
    >
    ...
    <div className="space-y-4">
      {todaysWorkouts.length > 0 && (
        <div className="space-y-3">
          <p className="text-sm font-medium text-muted-foreground">Saved Workouts</p>
          {todaysWorkouts.map((w) => (
            <div key={w.id} className="bg-card rounded-xl border border-border/50 p-4">
              ...
              <div className="space-y-1">
                {w.exercises.map((ex, i) => (
                  <p key={i} className="text-xs text-muted-foreground">
                    {ex.exerciseName} — {ex.sets.length} sets · {ex.caloriesBurned} cal
                  </p>
                ))}
              </div>
              {w.notes && (
    ...
    <div className="space-y-6">
      <div className="bg-card rounded-xl border border-border/50 p-4 space-y-3">
        ...
        <div className="flex items-center justify-between">
          ...
          <p className="text-xs text-muted-foreground">Target: {profile?.weeklyWorkoutsTarget || 4}/week</p>
        </div>
      </div>
      ...
      <div className="bg-card rounded-xl border border-border/50 p-4">
        <button onClick={() => setHasPR(!hasPR)} className="w-full flex items-center justify-between">
          <div className="flex items-center gap-2">
            <Trophy className={cn("w-4 h-4", hasPR ? "text-yellow-500" : "text-muted-foreground")} />
            <p className="text-sm font-medium text-foreground">New Personal Record?</p>
          </div>
          <div className={cn("w-12 h-7 rounded-full transition-all flex items-center", hasPR ? "bg-primary justify-end" : "bg-muted justify-start")}>
      ...
      <div className="bg-card rounded-xl border border-border/50 p-4 space-y-3">
        <div className="flex items-center gap-2">
          ...
          <p className="text-sm font-medium text-foreground">Weight Check-in (optional)</p>
        </div>
        <input
          type="number"
          value={weightKg ?? ""}
