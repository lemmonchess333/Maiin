...
| built-held | ROUTE-PLANNING-01 (Mapbox `planRunningRoute`)                                 | **DECIDED 2026-07-17 (owner): Mapbox supersedes ORS** (ORS free tier needs commercial-use approval) and the **Run11 Pro gate is retained**. Built on `claude/route-planning` as a HELD PR — the `MAPBOX_DIRECTIONS_TOKEN` binding is itself the deploy gate: provision it (and #1636's RESEND key) before merging. Rollout: `docs/road-aware-route-planning.md`. |
