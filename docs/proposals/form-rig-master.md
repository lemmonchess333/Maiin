...
| Build-out batch 17 — `keyedPose` (per-leg, per-arm keyframes) → man-maker as a thirteen-key cycle, Turkish get-up as a seven-key reversible sequence; coverage 149 → 151                                                                                                 | `bodyRig.ts` + pins                                              | ✅ 2026-09-03               |
| Build-out batch 18 — the ANKLE: the foot is its own side piece (`footL`/`footR`, riding the shin unless posed) → seated calf raise; coverage 151 → 152 of 152 — every catalogue exercise has a demo                                                                      | `bodySideData.ts` + `bodyTypes.ts` + `bodyRig.ts` + pins         | ✅ 2026-09-03               |
| Placard player — a third mode beside the rep and the cycle: named positions held to a derived READ budget, tweened between, cue under the figure; muscle key from the demo's own fills; dips only                                                                        | `exerciseTempo.ts` + `bodyRig.ts` + `ExerciseRigDemo.tsx` + pins | ✅ 2026-09-03               |
