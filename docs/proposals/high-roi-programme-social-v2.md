...
| PR 6 PROGRAM-DELOAD-01           | **SHIPPED** (post-#1636)                | Built as demanded once #1636 resolved (2026-07-18): `applyDeloadWeek` / `revertDeloadWeek` as idempotent `applyProgramCommand` kinds (first client consumers of the callable) with a server `deloadEngine.js` mirror pinned by a parity cross-test; `deloadSnapshot` on programState powers the undo toast; DeloadBanner's reserved Apply CTA + `'applied'` telemetry delivered. No client-side whole-program overwrite was built.                                                                                                                     |
