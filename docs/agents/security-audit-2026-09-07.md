...
- **Release:** compare the deployed source of both affected comment callables with the uploaded `index.js` (including the CI SHA marker) and three helper files. The verifier was exercised locally with a matching synthetic archive and a stale archive; stale bytes are rejected. Actual production verification is a separate post-deploy gate.
...
| Dependencies | Fresh `npm audit`: production root tree **0 advisories**. Full root/dev tree 12 (1 low, 8 moderate, 3 high); Functions tree 16 (1 low, 10 moderate, 4 high, 1 critical). | Counts describe dependency trees, not reachable exploits. The critical Functions path is through the unused Realtime Database dependency; source has no RTDB caller. Existing Admin SDK major-migration constraints remain. Reassess each advisory when the lockfile changes; do not apply a blind force/downgrade. |
