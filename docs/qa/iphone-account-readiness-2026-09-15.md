...
Release integration: the latest `main` chart fix merged cleanly and the full app gate passed with 9,294 tests. Browser fixtures that create accounts now read the actual verification message from the strictly local Auth emulator, apply its one-time code, and use the app's verification confirmation before continuing. This preserves the required signup gate and the original coachmark, offline-account-switch, water and capture assertions; no tests are skipped to accommodate verification.
