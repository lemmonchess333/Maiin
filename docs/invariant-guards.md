...
| `tsconfig.scripts.json` (in `tsc -b`)                                 | every `scripts/*.ts` and `e2e/**/*.ts` file compiles against the real `src/` exports, so a helper deleted or renamed in `src/` fails the build instead of the capture workflow's seed step (a `seed-rich-user.ts` import of a deleted export shipped green through unit, lint and e2e CI and broke only when the emulator rig ran it) |
