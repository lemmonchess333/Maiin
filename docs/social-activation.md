...
| ---------------------------------------------------- | ------------------------------------------ | ----------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Solo-first layout**                                | — (this IS the dormant-network experience) | shown while user follows **0 accounts** (cold-start)                    | `followingCount === 0` in `Social.tsx` — `isSoloUser` was DELETED in #1778 (its partner-count predicate would have blanked the feed for most established users; see the tombstone in `socialGates.ts`) |
| **Following feed**                                   | hidden (not an empty list)                 | user follows **≥3 active accounts**                                     | `shouldShowFollowingFeed(activeFollowCount)`                                                                                                                                                           |
| ~~Crew surfaces~~                                    | —                                          | — crews were RETIRED in #1700; `shouldShowCrewSurface` no longer exists | — (row kept for the historical record)                                                                                                                                                                 |
