...
### Correction — registration DOES help, and the earlier number was wrong

STATUS 03v recorded that a translation-registration pass made frame
alignment worse (9.3% → 8.7%) and concluded it "cannot be fixed
downstream". **That conclusion was wrong**, and it was repeated into the
extractor's comments, a commit message and the generator prompt before
anyone re-measured it.

It was a statement about a bad implementation, not about registration.
That version searched a downsampled mask, and applied whatever shift it
found without checking whether the shift helped — so it moved frames
that should have been left alone.

Searching at full resolution and applying a shift **only when it beats
leaving the frame alone** helps both cases. Measured on the output files
rather than on an intermediate:

|                                      | before | after |
| ------------------------------------ | ------ | ----- |
| card (six panels, separate drawings) | 10.2%  | 23.1% |
| per-position (edits of one original) | 38.2%  | 47.6% |

The per-position numbers start far higher because those frames differ
mostly by translation, which is what a shift corrects. The card's panels
also differ in SCALE, which no translation reaches — that half of the
original reasoning held, and is why the card still ends up at 23%.

The lesson is the one this repo keeps writing down in other forms: a
measurement of one implementation is not a measurement of the idea. The
first pass should have been reported as "my registration did not help",
not "registration does not help".

### The first real per-position set

Six images, generated by editing the first, at 1254px square:

|                                         | card   | per-position |
| --------------------------------------- | ------ | ------------ |
| figure region shared by all frames      | 301 px | 754 px       |
| upscale to the 900 px the phone renders | 3.0x   | 1.2x         |
| equipment overlap, after registration   | 23.1%  | 47.6%        |

The shoulder plate is gone — that was the generator's, and asking for
continuous anatomy fixed it. The generator also burned a caption into
every frame despite the prompt asking for no text, so the extractor now
finds and paints those out: content rows are grouped into bands, and a
