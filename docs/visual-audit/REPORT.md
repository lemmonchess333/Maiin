...
4. ~~**Empty-state quality — two text-only empties on a RICH account:** the "Welcome to Tropos!" checklist card still renders for a user with 18 workouts + an active race plan, and the Performance card says "appears after your first logged session" despite seeded sessions (rollup hasn't run in the rig, but the empty itself is an undesigned grey-dash + sentence).~~ Both are prime cold-start surfaces (CLAUDE.md: cold-start is a most-seen state). **Both halves fixed.** Performance (Wave3 F): the no-doc branch now renders the hexagon `EmptyState` primitive with a "Start a workout" CTA (`PerformanceHeroCard.tsx`). Welcome checklist (#972, audit #7): visibility is now data-derived via `shouldShowWelcomeChecklist` (`activationFraming.ts`) — suppressed at ≥3 workouts, past the 14-day activation window, on completion, or on dismiss; the 18-workout rich account is hidden (pinned by `activationFraming.test.ts`). See ranked #7.
...
7. ~~**Stale cold-start artifacts on rich accounts** — "Welcome to Tropos!" checklist still rendering for an account with months of data.~~ — **fixed** (#972, audit #7): the checklist's visibility is now data-derived (`shouldShowWelcomeChecklist` in `activationFraming.ts`), not a bare localStorage dismiss-flag. It hides when the user has ≥3 workouts, is past the 14-day activation window (null `createdAt` fails closed), has completed the activation loop, or dismissed it. The exact "18 workouts" audit case is pinned by `activationFraming.test.ts` (→ `false`). (`01-home-top`)
