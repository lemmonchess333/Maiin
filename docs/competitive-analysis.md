...
2. **Weight: trend-vs-noise overlay + self-correcting goal-date projection + milestone framing** — render raw daily dots against the smoothed trend line (show a water spike _not_ moving the trend), and project "goal by [date]" that recomputes each weigh-in from trend slope; tie milestone celebration to existing plateau detection so a stall reads "expected." _Why:_ Happy Scale's entire moat; strengthens Tropos's adaptive-TDEE story. _Where:_ `fetchBodyweightLogs` + `utils/weightTrend`, progress charts (`useBodyweightTrend` was deleted 2026-07-27 — uncalled; build on the raw readers). _Effort:_ low-med.
   - **STATUS 2026-06-02 — SHIPPED (Hist5d arc).** `TrendWeight.tsx` already renders raw `actual` dots + EMA-smoothed `trend` line (`calculateEMA`), a goal `ReferenceLine`, and a **slope-based goal-date projection** that honestly suppresses itself when the trend is flat or moving away from goal (`:146`).
   - **STATUS 2026-06-03 — "hide the number" mode SHIPPED (#984).** The last open piece is now closed: an OFF-by-default `hideWeightNumber` preference suppresses the raw figure app-wide (home `WeightStepsTiles` tile + Progress `TrendWeight` chart — header readouts, Y-axis ticks, tooltip values, goal-label figure) and shows direction/trend + goal-progress copy instead. The trend line, EMA, goal line and projection still render. The whole Happy Scale / Withings weight moat (overlay + projection + anti-anxiety mode) is shipped.
...
9. **Pre-run "Workout Briefing" + warmer audio coaching** — a short pre-session brief ("today: 5×1k, here's why") and warmer guided-run voice. _Why:_ Runna/NRC adherence win, cheap. _Where:_ `guidedRun.ts`, `useSessionPlayer`, `useAudioCues`. _Effort:_ low-med.
10. **Nutrient-gap coaching + verified-data layer** — "you're low on X this week → log Y" tied to the health score; lean on a verified food-data source for trust. _Why:_ Cronometer's Oracle; maps onto existing insights. _Where:_ `healthScore.ts`, food data source (note: `nutritionInsights.ts` was deleted 2026-07-25 as an orphan). _Effort:_ med.
...
14. **"Hide the number" anti-anxiety weight mode** (Withings) — fits Tropos's calm brand. _Effort:_ low. — **STATUS 2026-06-03: SHIPPED (#984).** OFF-by-default `hideWeightNumber` preference; suppresses the raw figure in `WeightStepsTiles` + `TrendWeight` (direction/trend + goal progress shown instead), trend/projection unaffected. The overlay + projection it sits alongside shipped earlier (Hist5d, see item 2).
