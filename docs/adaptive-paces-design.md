...
| `useSessionPlayer` (was `useIntervalWorkout`)/`IntervalDisplay`                      | `workPace` currently undefined                                                         | populate from resolver (display only; state machine ignores pace) | none               |
| `useSessionPlayer` (was `useGuidedRun`)/`GuidedRunOverlay`                           | segments are type-only, no pace                                                        | optional: add `targetPace?` per segment from resolver             | none               |
