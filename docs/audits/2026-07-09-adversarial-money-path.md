...
| F8  | Stale `appleSubscriptions` binding on deletion     | Low             | **REMEDIATED 2026-07-11**         | Executor step 3c: tombstone-then-delete — the binding identity is HMAC-tombstoned into `deletedBillingIdentities` (which restore already refuses) BEFORE the binding doc is removed, closing the F7 re-open a naive delete would create; fail-safe keeps the binding if `BILLING_HMAC_SECRET` is absent. `deleteMyAccount` gains the secret binding (snapshot-pinned) | `accountDeletion.test.js` F8 pair (order pin + fail-safe)   | CI deploy on merge; Console spot-check = operator | Negligible                                                                 |
