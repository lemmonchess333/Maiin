...
test.describe('Security & CSP', () => {
  ...
  test('CSP restricts frame sources to Stripe and Firebase', async ({ page }) => {
    await page.goto('/');
    const csp = await page.locator('meta[http-equiv="Content-Security-Policy"]').getAttribute('content');
    expect(csp).toContain('frame-src https://js.stripe.com https://*.firebaseapp.com');
  });
  ...
  test('no inline scripts in body (CSP compliance)', async ({ page }) => {
    ...
    const bodyScripts = await page.locator('body script:not([src]):not([type="module"])').count();
